...
/obj/machinery/trading_card_button
	...
	var/obj/effect/bar_hud_display/display_panel_ref
	///Typepath of the display panel generated.
	var/display_panel_type = /obj/effect/bar_hud_display/tcg_mana
...
/obj/machinery/trading_card_button/proc/handle_choice(choice, mob/user)
	...
	switch(choice)
		if("Set Mana")
			input_value = tgui_input_number(user, "Please input total mana", "Set mana", display_panel_ref.active_dots, display_panel_ref.dot_slots, 0)
			if(!isnull(input_value))
				display_panel_ref.active_dots = input_value
		if("Set Mana Slots")
			input_value = tgui_input_number(user, "Please input total mana slots", "Set mana slots", display_panel_ref.dot_slots, 10, 1)
			if(input_value)
				display_panel_ref.dot_slots = input_value
		if("Next Turn")
			if (display_panel_ref.dot_slots <= 9)
				display_panel_ref.dot_slots += 1
			display_panel_ref.active_dots = display_panel_ref.dot_slots
...
/obj/machinery/trading_card_button/health
	...
	display_panel_type = /obj/effect/bar_hud_display/tcg_health
	panel_offset_x = -1
...
/obj/machinery/trading_card_button/health/handle_choice(choice, mob/user)
	...
	switch(choice)
		if("Set Life")
			input_value = tgui_input_number(user, "Please input total life", "Set life", display_panel_ref.active_dots, display_panel_ref.dot_slots, 0)
			if(!isnull(input_value))
				display_panel_ref.active_dots = input_value
		if("Inflict Damage")
			display_panel_ref.active_dots -= tgui_input_number(user, "Please input total damage", "Inflict damage", 1, display_panel_ref.dot_slots, 0)

///A variant of the hud display panel for mana, shown in blue, with only one column.
/obj/effect/bar_hud_display/tcg_mana
	name = "life shard panel"
	pixel_x = 9

	active_dots = 1
	dot_slots = 1
	individual_dot_offset_x = -10
	dot_icon_state = "gem_blue"
